...
const MessagesRealtime = ({ selectedMatchId, onBack, match }) => {
  ...
  useEffect(() => {
    const initConversation = async () => {
      ...
      try {
        // Get or create conversation
        const { data: conversation, error } = await realtimeChatService.getOrCreateConversation(match.id)
        ...
        const { data: existingMessages, error: messagesError } = await realtimeChatService.getMessages(conversation.id, 100)
  ...
  useEffect(() => {
    ...
    unsubscribeMessagesRef.current = realtimeChatService.subscribeToMessages(
      ...
      (newMessage, eventType) => {
        if (eventType === 'update') {
          // Message was updated (e.g., marked as read)
          setMessages(prev => 
            prev.map(msg => 
    ...
    realtimeChatService.markAsRead(conversationId, currentUser.id)
  ...
  const handleSendMessage = async (e) => {
    e.preventDefault()
    if (!messageText.trim() || !conversationId || !currentUser?.id) return
